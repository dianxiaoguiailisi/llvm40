...
bool CollectCondBranchHints::instrumentCondBranch(Instruction *I, Value *cond) {
    ...
    // Get or insert the function (FunctionCallee is used here)
    FunctionType *FuncType = FunctionType::get(VoidTy, {I1Ty}, false);
    FunctionCallee FuncCollectCondBranchHints = M->getOrInsertFunction("__collect_cond_branch_hints", FuncType);

    // Directly use the FunctionCallee to create a call
